...
export const ProductDetail: React.FC = () => {
  ...
  const productInCart = useMemo(
    () => cart.find((data: any) => String(data.pid) === String(productId)),
    [cart, productId]
  );
      ...
      {!isLoading ? (
        <Box className=" w-full md:w-2/4 lg:ml-8 md:ml-6 md:mt-0 mt-6">
          <Box className="border-b border-gray-200 pb-6">
            <Typography className=" lg:text-3xl text-2xl font-semibold  text-gray-800 mt-2">
              {product?.title}
            </Typography>
          </Box>
          ...
          <Typography className="mt-5 mr-5" variant="h3">
            ${product?.price}
          </Typography>

          <Box>
            <Typography className=" text-base lg:leading-tight leading-normal text-gray-600 mt-7">
              {product?.description}
            </Typography>
            <Typography className="text-base leading-4 mt-7 text-gray-600">
              Product Code: {unique_id}
            </Typography>

            <Typography className="md:w-96 text-base leading-normal text-gray-600 mt-4">
              Lorem ipsum dolor sit, amet consectetur adipisicing elit.
              Exercitationem doloribus minus quasi, nam error enim magni
              temporibus optio dolor mollitia nesciunt rerum corrupti est. Id
              sequi dolorum libero atque odit.
            </Typography>
          </Box>
